...
void putchar(char c) {
    ...
    if (c == '\n') {
        ...
        VGA_MEMORY[cursor_y * SCREEN_WIDTH + cursor_x] = ' ' | (color_byte << 8);
    } else {
        VGA_MEMORY[cursor_y * SCREEN_WIDTH + cursor_x] = c | (color_byte << 8);
        cursor_x++;
    }
...
void clear_chars(uint16_t start_pos, uint16_t count) {
    uint8_t color_byte = get_color();
